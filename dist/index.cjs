...
var custom_volatility = (duration, smoother) => pipe(mapWithLast((input, last) => input.div(last).minus(1)), custom_stddev(smoother), multiplyBy(new import_decimal.default(duration).sqrt()));
...
// src/indicators.ts
var import_decimal2 = __toModule(require("decimal.js"));
...
var custom_tr = (skipFirst = false) => makeStatefulMap(() => {
  ...
  return (candle) => {
    const last = prev;
    prev = candle;
    return last ? import_decimal2.default.max(candle.high.minus(candle.low), candle.high.minus(last.close).abs(), candle.low.minus(last.close).abs()) : skipFirst ? ZERO : candle.high.minus(candle.low);
  };
});
...
var vi = (condition) => pipe(fork(pipe(volume, mapWithLast(condition, false)), pipe(closeprice, mapWithLast((close, last) => close.minus(last).div(last), ZERO))), flatMap((check, close) => check ? close : ZERO), scan((vi2, extra) => vi2.plus(vi2.times(extra)), new import_decimal2.default(1e3)));
...
var fisher = (period) => pipe(medprice, stoch(identity(), max(period), min(period)), scan((res, val) => val.minus(0.5).times(0.66).plus(res.times(0.67)), ZERO), map((val) => val.gt(0.99) ? new import_decimal2.default(0.999) : val.lt(-0.99) ? new import_decimal2.default(-0.999) : val), map((val) => ONE.plus(val).div(ONE.minus(val)).ln()), scan((fisher2, val) => val.times(0.5).plus(fisher2.times(0.5)), ZERO), forkWithLag(1, ZERO));
