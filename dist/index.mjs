...
var decay = (period) => scan(
  (decay2, value) => Decimal.max(value, decay2.minus(1 / period), ZERO),
  ZERO
);
var edecay = (period) => scan(
  (decay2, value) => Decimal.max(value, decay2.times(period - 1).div(period), ZERO),
  ZERO
);
...
var ha_high = pipe(
  ...
  flatMap((high, open, close) => Decimal2.max(high, open, close))
);
var ha_low = pipe(
  fork(lowprice, ha_open, ha_close),
  flatMap((low, open, close) => Decimal2.min(low, open, close))
);
...
var vi = (condition) => pipe(
  ...
  scan((vi2, extra) => vi2.plus(vi2.times(extra)), new Decimal2(1e3))
);
...
var fisher = (period) => pipe(
  ...
  map(
    (val) => val.gt(0.99) ? new Decimal2(0.999) : val.lt(-0.99) ? new Decimal2(-0.999) : val
  ),
